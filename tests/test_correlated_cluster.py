...
def test_generate_correlated_cluster_shapes_and_correlation(structure: str):
    X, meta = generate_correlated_cluster(
        n_samples=200, size=5, rho=0.6, structure=structure, random_state=123
    )
...
def test_toeplitz_accepts_negative_rho():
    ...
    X, meta = generate_correlated_cluster(
        n_samples=150, size=6, rho=-0.4, structure="toeplitz", random_state=1
    )
    C = meta["corr_matrix"]
    assert C.shape == (6, 6)
