...
class TestBuildCorrelationMatrix:
    ...
    def test_equicorr_shape_and_dtype(self):
        ...
        R = build_correlation_matrix(n_features=5, rho=0.7, structure="equicorrelated")
        assert R.shape == (5, 5)
        assert R.dtype == np.float64

    def test_toeplitz_shape_and_dtype(self):
        """Toeplitz matrix has correct shape and dtype."""
        R = build_correlation_matrix(n_features=4, rho=0.5, structure="toeplitz")
        assert R.shape == (4, 4)
        assert R.dtype == np.float64
    ...
    def test_equicorr_diagonal_is_one(self):
        ...
        R = build_correlation_matrix(n_features=6, rho=0.6, structure="equicorrelated")
        assert np.allclose(np.diag(R), 1.0)

    def test_toeplitz_diagonal_is_one(self):
        """Toeplitz matrix has ones on diagonal."""
        R = build_correlation_matrix(n_features=5, rho=0.7, structure="toeplitz")
        assert np.allclose(np.diag(R), 1.0)
    ...
    def test_equicorr_offdiagonal_constant(self):
        """Equicorrelated matrix has constant off-diagonal values equal to rho."""
        rho = 0.75
        R = build_correlation_matrix(n_features=6, rho=rho, structure="equicorrelated")

        # All off-diagonal elements should equal rho
        mask = ~np.eye(6, dtype=bool)
        assert np.allclose(R[mask], rho)

        # Check specific pairs
        assert np.allclose(R[0, 1], rho)
        assert np.allclose(R[2, 4], rho)
        assert np.allclose(R[1, 5], rho)

    def test_toeplitz_exponential_decay(self):
        """Toeplitz matrix shows exponential decay with distance."""
        rho = 0.8
        R = build_correlation_matrix(n_features=6, rho=rho, structure="toeplitz")
    ...
    def test_toeplitz_negative_rho_alternating_signs(self):
        ...
        rho = -0.5
        R = build_correlation_matrix(n_features=5, rho=rho, structure="toeplitz")
    ...
    def test_equicorr_symmetric(self):
        ...
        R = build_correlation_matrix(n_features=5, rho=0.8, structure="equicorrelated")
        assert np.allclose(R, R.T)

    def test_toeplitz_symmetric(self):
        """Toeplitz matrix is symmetric."""
        R = build_correlation_matrix(n_features=6, rho=0.6, structure="toeplitz")
        assert np.allclose(R, R.T)
    ...
    def test_single_feature_returns_scalar_one(self):
        ...
        R_eq = build_correlation_matrix(n_features=1, rho=0.5, structure="equicorrelated")
        R_tp = build_correlation_matrix(n_features=1, rho=0.5, structure="toeplitz")
    ...
    def test_zero_correlation_equicorr(self):
        """Equicorrelated with rho=0 produces identity matrix."""
        R = build_correlation_matrix(n_features=4, rho=0.0, structure="equicorrelated")
        assert np.allclose(R, np.eye(4))

    def test_zero_correlation_toeplitz(self):
        """Toeplitz with rho=0 produces identity matrix."""
        R = build_correlation_matrix(n_features=4, rho=0.0, structure="toeplitz")
        assert np.allclose(R, np.eye(4))

    def test_very_high_correlation_equicorr(self):
        """Equicorrelated with rho close to 1 is numerically stable."""
        R = build_correlation_matrix(n_features=5, rho=0.999, structure="equicorrelated")
    ...
    def test_very_high_correlation_toeplitz(self):
        """Toeplitz with rho close to 1 is numerically stable."""
        R = build_correlation_matrix(n_features=5, rho=0.999, structure="toeplitz")
    ...
    def test_very_low_correlation_both_structures(self):
        """Very low rho (close to 0) is numerically stable."""
        for structure in ["equicorrelated", "toeplitz"]:
            R = build_correlation_matrix(n_features=4, rho=0.001, structure=structure)
    ...
    def test_large_matrix_performance(self):
        ...
        R_eq = build_correlation_matrix(n_features=50, rho=0.5, structure="equicorrelated")
        R_tp = build_correlation_matrix(n_features=50, rho=0.5, structure="toeplitz")
    ...
    def test_invalid_n_features_zero_raises(self):
        ...
        with pytest.raises(ValueError, match="n_features must be positive"):
            build_correlation_matrix(n_features=0, rho=0.5, structure="equicorrelated")

    def test_invalid_n_features_negative_raises(self):
        """Negative features raises ValueError."""
        with pytest.raises(ValueError, match="n_features must be positive"):
            build_correlation_matrix(n_features=-5, rho=0.5, structure="toeplitz")

    def test_equicorr_rho_equals_one_raises(self):
        """Equicorrelated with rho=1 raises ValueError (not PD)."""
        with pytest.raises(ValueError, match="Invalid rho.*require.*< 1"):
            build_correlation_matrix(n_features=3, rho=1.0, structure="equicorrelated")

    def test_equicorr_rho_too_negative_raises(self):
        """Equicorrelated with rho <= -1/(n-1) raises ValueError."""
        # For n=4, lower bound is -1/3 ≈ -0.333
        with pytest.raises(ValueError, match="Invalid rho.*require.*< 1"):
            build_correlation_matrix(n_features=4, rho=-0.5, structure="equicorrelated")

    def test_toeplitz_rho_equals_one_raises(self):
        """Toeplitz with |rho|=1 raises ValueError (not PD)."""
        with pytest.raises(ValueError, match="Invalid rho.*require.*< 1"):
            build_correlation_matrix(n_features=5, rho=1.0, structure="toeplitz")

        with pytest.raises(ValueError, match="Invalid rho.*require.*< 1"):
            build_correlation_matrix(n_features=5, rho=-1.0, structure="toeplitz")

    def test_toeplitz_rho_exceeds_one_raises(self):
        """Toeplitz with |rho| > 1 raises ValueError."""
        with pytest.raises(ValueError, match="Invalid rho.*require.*< 1"):
            build_correlation_matrix(n_features=3, rho=1.5, structure="toeplitz")
    ...
    def test_positive_definite_equicorr(self):
        """Equicorrelated matrix is positive definite for valid rho."""
        R = build_correlation_matrix(n_features=5, rho=0.7, structure="equicorrelated")
    ...
    def test_positive_definite_toeplitz(self):
        """Toeplitz matrix is positive definite for |rho| < 1."""
        for rho in [0.3, 0.7, -0.3, -0.7]:
            R = build_correlation_matrix(n_features=5, rho=rho, structure="toeplitz")
    ...
    def test_determinant_positive_both_structures(self):
        ...
        for structure in ["equicorrelated", "toeplitz"]:
            R = build_correlation_matrix(n_features=4, rho=0.6, structure=structure)
            det = np.linalg.det(R)
    ...
    def test_reproducibility(self):
        ...
        R1 = build_correlation_matrix(n_features=5, rho=0.65, structure="equicorrelated")
        R2 = build_correlation_matrix(n_features=5, rho=0.65, structure="equicorrelated")
        assert np.allclose(R1, R2)

    def test_equicorr_independent_of_feature_order(self):
        """Equicorrelated matrix values don't depend on feature indices."""
        R = build_correlation_matrix(n_features=6, rho=0.8, structure="equicorrelated")
    ...
    def test_toeplitz_depends_only_on_distance(self):
        ...
        R = build_correlation_matrix(n_features=8, rho=0.7, structure="toeplitz")
